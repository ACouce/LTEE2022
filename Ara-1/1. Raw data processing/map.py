#!/usr/bin/python
#this script maps insertion sites to the list of loci of the reference genome. It needs to be called by main_miseq.sh.
import re
import sys
...
def main():
	 file=open(inputname)
	 ref=open("parsed_R606genoscope_I.txt")	#the list of loci of the reference genome
	 ...
	 itg=0
		...
		while (1):
			...
			if not refline:
				ref.close()
				ref=open("parsed_R606genoscope_I.txt")
				if flag==0:
					itg+=1
				break
