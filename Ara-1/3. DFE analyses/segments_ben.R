# .. This script plots the most the beneficial mutations in each background and their effects in the other backgrounds
...
arrows(1,Repi$R[1:nrow(Repi)],2,Repi$M[1:nrow(Repi)], col= gray (0, .5), lwd=0.75, length=0.09, angle=25)
...
arrows(2,Tepi$M[1:nrow(Tepi)],1,Tepi$R[1:nrow(Tepi)], col= rgb(0.8,0.43,0.42, .5), lwd=0.75, length=0.09, angle=25)
...
arrows(1,Tepi$M[1:nrow(Tepi)],2,Tepi$K[1:nrow(Tepi)], col= rgb(0.8,0.43,0.42, .5), lwd=0.75, length=0.09, angle=25)
...
# .. add arrows connecting the above points to the corresponding fitness values in the 2K background 
arrows(2,Fepi$K[1:nrow(Fepi)],1,Fepi$M[1:nrow(Fepi)], col=rgb(0.42,0.57,0.8, .5), lwd=0.75, length=0.09, angle=25)
