# ..  This script provides a neutral expectation for how well resampled DFEs capture the beneficial mutations becoming numerically dominant in the LTEE.
...
label<-'sims'									# flag to enable using 'repre.R' both with the real and the simulated data

# .. load the table with the beneficial mutations from simulated DFEs corresponding to the ancestor
Ntabs<-read.table(file = 'Rcontrol.dat', sep = "\t", header = TRUE)

# .. define parameters corresponding to the ancestor
...
source('repre.R')
write.table(rep, 'Rrep_sims.dat', row.names = FALSE, sep = "\t",  col.names = TRUE)	# write table with representativity results (fraction of drivers captured by the DFE)

# .. load the table with beneficial mutations from the simulated DFE at 2K
Ntabs<-read.table(file = 'Tcontrol.dat', sep = "\t", header = TRUE)

# .. define parameters corresponding to 2K
parthres<-1
genth<-2000
lins<-c('p2')
...
# .. run the script that estimates representativity with the above details
source('repre.R')
write.table(rep, 'Trep_sims.dat', row.names = FALSE, sep = "\t",  col.names = TRUE)	# write table with representativity results (fraction of drivers captured by the DFE)

# .. load the table with beneficial mutations from the simulated DFE at 15K
Ntabs<-read.table(file = 'Fcontrol.dat', sep = "\t", header = TRUE)
...
# .. run the script that estimates representativity with the above details
source('repre.R')
write.table(rep, 'Frep_sims.dat', row.names = FALSE, sep = "\t",  col.names = TRUE)	# write table with representativity results (fraction of drivers captured by the DFE)
