...
R<-c(7.8,15.2,21.5,28.3,34.6)
Ap2_2K<-c(6.0,13.3,19.6,26.1,32.7)
Ap2_15K<-c(8.7,15.9,22.2,28.6,35.7)
generations<-eval(parse(text = labels[sour]))  #for this set of experiments we measured actual generations, but switching to the ideal values makes a very small difference (6.64 gen./day)
...
for (i in 1:5) {
    cons_wt[i,1]<-quantile(list_neutral[list_neutral[,1]>mincov,i], 0.25, na.rm='TRUE')
}
