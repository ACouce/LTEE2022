#This script takes the unique list of alleles generated by 'table.R', and pools alleles at the level of sub-genic regions (each locus was divided into five equally sized segments, while requiring a minimum size of 100 bp per segment).
...
# .. read the reference file with the genome divided at the level of sub-genic regions
ORFnames<-read.table('parsed_R606genoscope_IUD.txt', sep="\t", header=FALSE, as.is=TRUE, comment.char = "") #The default is comment.char = "#"!!
...
# .. remove data from LB library (t0)
table<-table[,c(1:2,4:11)]
